library(matrixStats);library(limma);library(MASS);library(ape); library(RColorBrewer)
...
mst.of.classification <- function (x, k=6,color,...) {
    #   x <- t(x)
    ...
    m <- mst(graph.adjacency(as.matrix(dist(z))))
...
pseudotimeprog.foo <-function(x,k=10,color, x.reverse=F,...){
    r <- prcomp(t(x))
    ...
    endnode <- names(which(colSums(m!=0)==1))
    branchnode <- setdiff(endnode,c(t.names[1],t.names[length(t.names)]))
    t.names <- t.names[!t.names %in% branchnode]
    
    y2d <-y[names(r$cluster)[r$cluster %in% as.numeric(sub("t","",t.names))],1:2]
    #y2d <-y2d[order(y2d[,1]),]
    z2d <-z[,1:2]
    z2d <-z2d[t.names,]
    ...
    return(data.frame(y[names(r$cluster)[r$cluster %in% as.numeric(sub("t","",t.names))],1:2],pseudotime)[order(pseudotime),])
